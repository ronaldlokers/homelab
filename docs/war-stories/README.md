...
- **[The Revoke That Waited Four Hours to Break Anything](revoke-that-only-broke-things-four-hours-later.md)** - Revoking PUBLIC's CONNECT on the shared `postgres` database broke CloudNativePG replica rejoin; every health signal stayed green because the broken path only runs during a failure
